...
export function metaSnippet(text: string, maxLength: number = META_SNIPPET_MAX_LENGTH): string {
  ...
  // Too long even at one sentence: cut to the last whole word that fits
  // alongside the ellipsis, rather than through the middle of one.
  const room = maxLength - 1;
  const clipped = firstSentence.slice(0, room);
