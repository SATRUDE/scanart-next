  ...
  return (
    <div>
      <div className="aspect-[3/4] overflow-hidden bg-neutral-50 rounded cursor-pointer relative" onClick={() => setShowLightbox(true)}>
        ...
        {hasMultiple && (
          <>
            <Button aria-label={t.gallery.previousImage} size="icon" variant="ghost" className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white" onClick={(e) => { e.stopPropagation(); goTo(selectedIndex - 1); }}>
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button aria-label={t.gallery.nextImage} size="icon" variant="ghost" className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white" onClick={(e) => { e.stopPropagation(); goTo(selectedIndex + 1); }}>
      ...
      {showLightbox && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={() => setShowLightbox(false)}>
          <Button aria-label={t.gallery.closeViewer} size="icon" variant="ghost" className="absolute top-4 right-4 text-white hover:bg-white/10" onClick={() => setShowLightbox(false)}>
            <X className="h-6 w-6" />
          </Button>
          ...
          {hasMultiple && (
            <>
              <Button aria-label={t.gallery.previousImage} size="icon" variant="ghost" className="absolute left-4 top-1/2 -translate-y-1/2 text-white hover:bg-white/10" onClick={(e) => { e.stopPropagation(); goTo(selectedIndex - 1); }}>
                <ChevronLeft className="h-6 w-6" />
              </Button>
              <Button aria-label={t.gallery.nextImage} size="icon" variant="ghost" className="absolute right-4 top-1/2 -translate-y-1/2 text-white hover:bg-white/10" onClick={(e) => { e.stopPropagation(); goTo(selectedIndex + 1); }}>
